...
const ConsistencyDetailPage: React.FC<ConsistencyDetailPageProps> = ({ seriesId, onNavigate }) => {
    ...
    return (
        <MyLabLayout activePage="mylab/consistency" onNavigate={onNavigate}>
             <div className="mb-6">
                <h1 className="text-2xl font-semibold tracking-tight text-neutral-900 dark:text-white">{series.name}</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    {/* Fornadas associadas */}
                    <div className="rounded-xl border border-neutral-200 bg-white dark:bg-neutral-800 p-6 shadow-sm">
                        <h3 className="text-lg font-medium text-neutral-900 dark:text-neutral-100 mb-4">Fornadas Associadas ({relatedBakes.length})</h3>
                        {relatedBakes.length > 0 ? (
                            <div className="space-y-3">
                                {relatedBakes.map(bake => (
                                    <div key={bake.id} onClick={() => onNavigate('batch', bake.id)} className="cursor-pointer p-3 rounded-md bg-neutral-50 dark:bg-neutral-700/50 hover:bg-neutral-100 dark:hover:bg-neutral-700">
                         ...
                         <div className="mt-4 pt-4 border-t border-neutral-200 dark:border-neutral-700 flex flex-col sm:flex-row items-center gap-3">
                            <select value={selectedBake} onChange={e => setSelectedBake(e.target.value)} className="w-full flex-grow rounded-md border-neutral-300 bg-white dark:bg-neutral-700 text-sm py-2">
                    ...
                    {/* Notas */}
                    <div className="rounded-xl border border-neutral-200 bg-white dark:bg-neutral-800 p-6 shadow-sm">
                        <h3 className="text-lg font-medium text-neutral-900 dark:text-neutral-100 mb-4">Notas da Série</h3>
                        {isEditingNotes ? (
                             <div>
                                <textarea rows={5} value={notes} onChange={e => setNotes(e.target.value)} className="w-full rounded-md border-neutral-300 bg-white dark:bg-neutral-700 text-sm" />
                                <div className="flex justify-end gap-2 mt-2">
                                    <button onClick={() => setIsEditingNotes(false)} className="text-sm font-semibold text-neutral-600 dark:text-neutral-300">Cancelar</button>
                                    <button onClick={handleSaveNotes} className="text-sm font-semibold text-lime-600 dark:text-lime-400">Salvar</button>
                                </div>
                             </div>
                        ) : (
                             <div onClick={() => setIsEditingNotes(true)} className="cursor-pointer prose-sm dark:prose-invert text-neutral-600 dark:text-neutral-300 min-h-[5rem]">
                    ...
                    <div className="rounded-xl border-2 border-dashed border-neutral-300 dark:border-neutral-700 p-6 text-center">
                        <SparklesIcon className="mx-auto h-8 w-8 text-neutral-400" />
                        <h3 className="mt-2 text-lg font-medium text-neutral-800 dark:text-neutral-200">Análise da IA</h3>
                ...
                <div className="lg:col-span-1">
                     <div className="rounded-xl border border-neutral-200 bg-neutral-50 dark:bg-neutral-800 p-6 shadow-sm sticky top-24">
                        <h3 className="text-lg font-medium text-neutral-900 dark:text-neutral-100 mb-4">Parâmetros do Teste</h3>
