...
const LevainLayout: React.FC<LevainLayoutProps> = ({ children, levainName, activeTab, onTabChange }) => {
  const navItems: { id: LevainDetailTab; label: string; icon: React.ReactNode }[] = [
    { id: 'summary', label: 'Resumo', icon: <BeakerIcon className="h-5 w-5" /> },
    { id: 'feedings', label: 'Alimentações', icon: <ListBulletIcon className="h-5 w-5" /> },
    { id: 'profile', label: 'Perfil', icon: <BookOpenIcon className="h-5 w-5" /> },
