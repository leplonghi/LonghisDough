...
const CommunityBatchCard: React.FC<CommunityBatchCardProps> = ({ batch, onNavigate }) => {
    ...
    return (
        <button
            ...
            className="h-full text-left flex flex-col justify-between rounded-xl border border-slate-200 bg-white p-6 shadow-md transition-all hover:shadow-xl hover:-translate-y-1"
        >
            <div>
                ...
                {batch.styleId && (
                     <p className="mt-1 text-sm font-semibold text-lime-600">
            ...
            <div className="mt-4 flex items-center justify-between text-sm">
                <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-semibold text-slate-700">{batch.hydrationPercentage}%</span>
                </div>
                {batch.ratingAverage && (
                     <div className="flex items-center gap-1">
                        <StarIcon className="h-4 w-4 text-yellow-400"/>
                        <span className="font-semibold text-slate-700">{batch.ratingAverage.toFixed(1)}</span>
                    </div>
                )}
            </div>
        </button>
