...
const PlansPage: React.FC<PlansPageProps> = ({ onGrantAccess }) => {
  ...
  const features = [
      ...
      { key: 'plans_page.feature_ads', free: true, pro: false, freeText: t('pro.feature_ads') },
      ...
      <div className="rounded-2xl bg-white p-6 shadow-lg ring-1 ring-slate-200/50 dark:border dark:border-slate-700/50 dark:bg-slate-800 sm:p-10">
        ...
        <div className="mt-10 overflow-x-auto">
            ...
            <thead>
                ...
                <th className="py-3 text-center text-sm font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">{t('plans_page.free_tier')}</th>
                <th className="rounded-t-lg bg-lime-50 py-3 text-center text-sm font-bold uppercase tracking-wider text-lime-700 dark:bg-lime-500/10 dark:text-lime-300">{t('plans_page.pro_tier')}</th>
                </tr>
            </thead>
            <tbody>
                {features.map(f => (
                    <FeatureRow 
                        key={f.key} 
                        label={t(f.key)} 
                        free={renderCheck(f.free, f.freeText)} 
